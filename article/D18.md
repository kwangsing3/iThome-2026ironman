<!-- ═══════════════════════════════════════════════════════════════
     ...
     ✅ 待補項目(已補齊)
       [x] §5 成本紀錄實跡:console 的 [llm] usage 展開(d18-sample01)，已掛在
           「分開記這四個數」那段下方;accountId/threadId 未入鏡。
       [ ] (選配)多行 usage 節選——目前的圖是單次呼叫，「一份報告拆成幾十次」
           仍只有文字宣稱;若日後要補，需能看出行數。
...
```mermaid
flowchart LR
    A[一大包輸入] --> B[切塊 split<br/>程式:依主題邊界]
    B --> C1[分析第 1 塊<br/>LLM]
    B --> C2[分析第 2 塊<br/>LLM]
    B --> C3[⋯第 N 塊<br/>LLM]
    C1 --> D[跨塊綜述 reduce<br/>LLM:找關聯、排榜]
    C2 --> D
    C3 --> D
    D --> E[一份報告]
```
