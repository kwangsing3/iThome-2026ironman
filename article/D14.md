<!-- ═══════════════════════════════════════════════════════════════
     ...
     定位：D14–18 是「AI 基礎應用」五天，本篇是入口（前兩週的第二個純 code 硬實例，
           另一個是 D6）。
     ...
     📌 定位：D14–18 用 Azure AI Foundry 當教學載體，不是三系統的上線平台
     ══════════════════════════════════════════════════════════════ -->
...
前面 D10–13 聊完怎麼在組織裡擺 AI、怎麼用設計避免打掉原有系統做 A/B 導入；從今天起會一路實作到系列後段，接下來五天（D14–18）換成「做給你看」——把 AI 應用的骨架，一塊一塊拼給你看。而第一塊，就是把你從 0 帶到「跑出第一個會呼叫工具的 AI」。

這天結束，你手上的就不是一段筆記，是**一個真的會動手的 AI**——這也是接下來 D15（MCP）、D16（記憶）、D17–18（分裂操作：工具分派 + 任務拆解）要一路長上去的底子。
