<!-- ═══════════════════════════════════════════════════════════════
     ...
     ⚠️ 這是 D15–17 密度最高的一天（校稿提醒：把「檔案落 Redis」控制在一個
     ...
     🔗 承先啟後（依 article-framework Tip 3）
       承前：D15 做出「會呼叫一個工具的 AI」→ 本篇給它記憶，讓工具用得上上下文。
...
<!-- 前導 TL;DR（前 3–5 行）＝鉤子 + 結論先行。
     鉤子：昨天那個會呼叫工具的 AI，其實下一句就忘了上一句。
     結論：LLM 本身無狀態，「記憶」是你把歷史訊息存起來、每次帶回去。用 Azure Redis
           存工作階段，再接上工具呼叫，就有一個記得上下文、會動手的 agent。 -->

昨天你做出一個會呼叫工具的 AI。但如果你多聊兩句就會發現：**它記不住**。你說「查 C9300-48P-A」它查了，接著你問「那它的無線版呢」，它一臉茫然——因為它根本不記得「它」是誰。
